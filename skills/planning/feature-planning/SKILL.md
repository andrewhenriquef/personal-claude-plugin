...
Every finding acted on must end with `docs/<TAG>/PRD.md` updated on disk — mechanical fixes applied directly, and every `interrogate` answer written back into the relevant PRD section (not just settled in conversation). This step isn't done until the PRD file itself reflects all three agents' resolved findings.
...
Once Step 5's story files are written, append a `## Questions & Answers` section to the end of `docs/<TAG>/PRD.md`, listing every question routed through `interrogate` during Steps 2 and 4 — not `prd-development`'s own internal flow, only ones that went through `interrogate` — in the order asked, each with the answer the user gave:
...
If a question was answered and later revised during the flow, keep only the final answer — don't list stale intermediate ones. Run this once, not after every `interrogate` call.
